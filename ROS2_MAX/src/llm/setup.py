...
setup(
    ...
    packages=find_packages(where='llm/llm', exclude=['test']),  # Automatically find packages in the gg_cnn/gg_cnn directory
    package_dir={'src': 'llm'},  # Correctly point to the inner gg_cnn directory for Python files
    data_files=[
    ...
    description='LLM package for P8 project',  # Package description
    license='todo',  # License for the package
    tests_require=['pytest'],  # Testing framework
    entry_points={
        'console_scripts': [
            'llm_interface = llm.llm_interface:main',  # Updated entry point to match your script
        ],
    },
)
